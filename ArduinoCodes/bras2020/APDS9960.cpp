 ...
 */
bool APDS9960::init(int8_t sdaPin, int8_t sclPin)
{
    wireObj = new SoftwareWire(sdaPin, sclPin);
    uint8_t id;
...
{
    wireObj->beginTransmission(APDS9960_I2C_ADDR);
    wireObj->write(val);
    if( wireObj->endTransmission() != 0 ) {
        return false;
    }
                                        ...
                                        unsigned int len)
{
    ...
    wireObj->write(reg);
    for(i = 0; i < len; i++) {
        wireObj->beginTransmission(val[i]);
    }
    if( wireObj->endTransmission() != 0 ) {
        return false;
    }
...
bool APDS9960::wireReadDataByte(uint8_t reg, uint8_t &val)
{
    ...
    /* Read from register */
    wireObj->requestFrom(APDS9960_I2C_ADDR, 1);
                                        ...
                                        unsigned int len)
{
    ...
    /* Read block data */
    wireObj->requestFrom(APDS9960_I2C_ADDR, len);
    while (wireObj->available()) {
        if (i >= len) {
            return -1;
        }
